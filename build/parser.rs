...
impl MavProfile {
    ...
    fn emit_mav_message_name(&self, enums: &Vec<TokenStream>, includes: &Vec<Ident>) -> TokenStream {
    ...
    fn emit_mav_message_serialize(&self, enums: &Vec<TokenStream>, includes: &Vec<Ident>) -> TokenStream {
        quote! {
...
impl MavMessage {
    ...
    fn emit_description(&self) -> TokenStream {
        ...
        let desc = format_ident!("{}", desc);
        quote!(#desc)
    }

    fn emit_serialize_vars_v1(&self) -> TokenStream {
        let ser_vars = self
            .fields
            .iter()
            .map(|f| f.rust_writer())
        ...
        if let Some(enum_name) = &self.enumtype {
            if let Some(dsp) = &self.display {
                if dsp == "bitmask" {
                    ...
                    let tmp = self
                        .mavtype
                        .rust_reader(quote!(let tmp), buf.clone());
                    ...
                    match stack.last() {
                        ...
                        Some(&MavXmlElement::Field) => {
                            match attr.key {
                                ...
                                b"display" => {
                                    field.display = Some(String::from_utf8(attr.value.to_vec()).unwrap());
                                }
                                _ => (),
                            }
                        }
...
impl MavXmlFilter {
    ...
    pub fn filter_extension(
        &mut self,
        _element: &Result<Event, quick_xml::Error>,
    ) -> bool {
        return true;
    }
    ...
    pub fn filter_extension(
        &mut self,
        element: &Result<Event, quick_xml::Error>,
    ) -> bool {
        match element {
            Ok(content) => {
                match content {
